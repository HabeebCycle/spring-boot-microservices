...
function testCircuitBreaker() {
    ...
    assertEqual "CLOSED" "$($EXEC wget product-composite-service:8080/actuator/health -qO - | jq -r .components.circuitBreakers.details.productService.details.state)"
    ...
    assertEqual "Fallback product 2" "$(echo "$RESPONSE" | jq -r .name)"
    ...
    assertEqual "HALF_OPEN" "$($EXEC wget product-composite-service:8080/actuator/health -qO - | jq -r .components.circuitBreakers.details.productService.details.state)"
    ...
    assertEqual "CLOSED_TO_OPEN"      "$($EXEC wget product-composite-service:8080/actuator/circuitbreakerevents/productService/STATE_TRANSITION -qO - | jq -r .circuitBreakerEvents[-3].stateTransition)"
    assertEqual "OPEN_TO_HALF_OPEN"   "$($EXEC wget product-composite-service:8080/actuator/circuitbreakerevents/productService/STATE_TRANSITION -qO - | jq -r .circuitBreakerEvents[-2].stateTransition)"
    assertEqual "HALF_OPEN_TO_CLOSED" "$($EXEC wget product-composite-service:8080/actuator/circuitbreakerevents/productService/STATE_TRANSITION -qO - | jq -r .circuitBreakerEvents[-1].stateTransition)"
}
