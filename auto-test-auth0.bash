...
function testCircuitBreaker() {
    ...
    EXEC="docker run --rm -it --network=service-network alpine"

    # First, use the health - endpoint to verify that the circuit breaker is closed
    assertEqual "CLOSED" "$($EXEC wget product-composite-service:8080/actuator/health -qO - | jq -r .components.circuitBreakers.details.product-service.details.state)"
    ...
    assertEqual "HALF_OPEN" "$($EXEC wget product-composite-service:8080/actuator/health -qO - | jq -r .components.circuitBreakers.details.product-service.details.state)"
    ...
    assertEqual "CLOSED_TO_OPEN"      "$($EXEC wget product-composite-service:8080/actuator/circuitbreakerevents/product-service/STATE_TRANSITION -qO - | jq -r .circuitBreakerEvents[-3].stateTransition)"
    assertEqual "OPEN_TO_HALF_OPEN"   "$($EXEC wget product-composite-service:8080/actuator/circuitbreakerevents/product-service/STATE_TRANSITION -qO - | jq -r .circuitBreakerEvents[-2].stateTransition)"
    assertEqual "HALF_OPEN_TO_CLOSED" "$($EXEC wget product-composite-service:8080/actuator/circuitbreakerevents/product-service/STATE_TRANSITION -qO - | jq -r .circuitBreakerEvents[-1].stateTransition)"
}
...
# Writer access token - read and write access
...
ACCESS_TOKEN=$(curl --request POST --url 'https://${AUTH0_DOMAIN}/oauth/token' --header 'Content-Type: application/json' \
--data '{"grant_type":"password", "username":"${AUTH0_EMAIL}", "password":"${AUTH0_PASSWORD}", "audience":"https://localhost:8443/product-composite", "scope":"openid email product:read product:write", "client_id":"${AUTH0_CLIENT_ID}", "client_secret":"${AUTH0_CLIENT_SECRET}"}' -s | jq -r .access_token)
...
READER_ACCESS_TOKEN=$(curl --request POST --url 'https://${AUTH0_DOMAIN}/oauth/token' --header 'Content-Type: application/json' \
--data '{"grant_type":"password", "username":"${AUTH0_EMAIL}", "password":"${AUTH0_PASSWORD}", "audience":"https://localhost:8443/product-composite", "scope":"openid email product:read", "client_id":"${AUTH0_CLIENT_ID}", "client_secret":"${AUTH0_CLIENT_SECRET}"}' -s | jq -r .access_token)
READER_AUTH="-H \"Authorization: Bearer $READER_ACCESS_TOKEN\""

assertCurl 200 "curl -k https://$HOST:$PORT/product-composite/$PROD_ID_REVS_RECS $READER_AUTH -s"

# Verify that a DELETE request by a reader - client with only read scope is FORBIDDEN, 403
